...
const generateMessageIDV2 = (userId) => {
    ...
    const word = 'DEVAR';
    const prefix = '#DEVICNTIK-';
    // Ambil bagian dari hash yang panjangnya sisanya setelah dikurangi panjang prefix dan word
    const remainingLength = 24 - prefix.length - word.length;
    ...
    // Sisipkan kata DEVAR ke posisi acak dalam partialHash, lalu tambahkan prefix #DEVAR-
    const insertPos = Math.floor(Math.random() * (remainingLength + 1));
    const finalID = prefix + partialHash.slice(0, insertPos) + word + partialHash.slice(insertPos);
    return finalID;
};
